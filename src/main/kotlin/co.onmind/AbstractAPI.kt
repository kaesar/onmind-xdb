...
open class AbstractAPI() {
    ...
    val apiSend: BiDiBodyLens<Back> = Body.auto<Back>().toLens()

    //fun Request.getBodyAsJson() = mapper.readValue(this.body(), MutableMap::class.java)

    fun sendSuccess(result: Map<String, Any?>) = apiSend.inject(Back(result), Response(Status.OK))
        //Response(Status.OK).header("Content -Type",ContentType.APPLICATION_JSON.value).body(mapper.writeValueAsString(result))

    fun sendSuccess(result: List<MutableMap<String, Any?>>?) = apiSend.inject(Back(result), Response(Status.OK))

    fun sendSuccess(message: String) =
        apiSend.inject(Back(true, "200", message), Response(Status.OK))

    fun sendError(message: String) =
        apiSend.inject(Back(false, Status.BAD_REQUEST.code.toString(), message), Response(Status.BAD_REQUEST))
